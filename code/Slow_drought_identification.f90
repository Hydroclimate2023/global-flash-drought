...
integer,parameter::td=4      ! drought duration threshold
integer n,i,j,k
...
integer cnt1,cnt2
real odur1(n),osev1(n),ospd1(n),drt1(n)
...
if(cnt1>0)then
   b(1)=real(cnt1-cnt)
   b(2)=real(sum(odur1(1:cnt1))-sum(odur(1:cnt)))
   b(3)=sum(osev1(1:cnt1))-sum(osev(1:cnt))
   b(4)=sum(ospd1(1:cnt2))
else
   b=0
endif
end
